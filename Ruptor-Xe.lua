...
-- Configuration
local VERSION = 131
local GITHUB_VERSION_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/RuptorX/main/version.txt"
local GITHUB_SCRIPT_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/RuptorX/main/RuptorX-Xe.lua"
local FOLDER_PATH = "RuptorX"
local SCRIPT_PATH = FOLDER_PATH .. "/RuptorX-Xe.lua"
...
local FILES = {
    ...
    TempRestarter = FOLDER_PATH .. "/temp_restarter.lua",
...
function SystemMaintenance.initFolders()
    if not isfolder(FOLDER_PATH) then
        makefolder(FOLDER_PATH)
        print("SYSMAINT: Created RuptorX folder")
    end
end

-- Check for improper shutdown
function SystemMaintenance.checkImproperShutdown()
    if isfile(FILES.NoShut) then
        local incrementContent = isfile(FILES.Increment) and readfile(FILES.Increment) or "0"
        local lastIncrement = tonumber(incrementContent) or 0
        
        if lastIncrement ~= 0 then
            if isfile(FILES.NoShut) then delfile(FILES.NoShut) end
            if isfile(FILES.Increment) then delfile(FILES.Increment) end
            
            Interpreter.notify("Improper Shutdown", "Warning: RuptorX was incorrectly shutdown, use *shutdown before leaving!", "warning")
            print("SYSMAINT: Improper shutdown detected, cleaned up lock files")
        end
    end
end

-- Initialize version file
function SystemMaintenance.initVersion()
    if not isfile(FILES.Version) then
        local versionData = game:GetService("HttpService"):JSONEncode({version = VERSION})
        writefile(FILES.Version, versionData)
        print("SYSMAINT: Created version.json")
    end
end

-- Start increment system
function SystemMaintenance.startIncrement()
    writefile(FILES.NoShut, "1")
    writefile(FILES.Increment, "0")
    ...
    SystemMaintenance.IncrementLoop = task.spawn(function()
        while SystemMaintenance.Active do
            ...
            writefile(FILES.Increment, tostring(SystemMaintenance.IncrementValue))
        end
    end)
...
function SystemMaintenance.checkForUpdates()
    local success, versionText = httpGetWithTimeout(GITHUB_VERSION_URL, 3)
    
    if not success then
        Interpreter.notify("Update Check Failed", "Warning: Couldn't check latest version, network congested!", "warning")
        print("SYSMAINT: Update check timed out")
        return
    end
    
    local latestVersion = tonumber(versionText)
    if not latestVersion then
        print("SYSMAINT: Invalid version format from GitHub")
        return
    end
    
    print("SYSMAINT: Current version: " .. VERSION .. ", Latest version: " .. latestVersion)
    
    if latestVersion > VERSION then
        if isfile(FILES.AutoUpdate) then
        ...
        print("SYSMAINT: RuptorX is up to date")
    end
end

-- Perform update
function SystemMaintenance.performUpdate(newVersion)
    print("SYSMAINT: Downloading update version " .. newVersion)
    
    local success, scriptContent = pcall(function()
        return game:GetService("HttpService"):GetAsync(GITHUB_SCRIPT_URL)
    end)
    
    if not success then
        Interpreter.notify("Update Failed", "Could not download update from GitHub", "error")
        print("SYSMAINT: Update download failed")
        return
    end
    
    writefile(SCRIPT_PATH, scriptContent)
    Interpreter.notify("Update Installed", "Version " .. newVersion .. " installed. Restarting...", "success")
    print("SYSMAINT: Update installed, initiating restart")
...
function SystemMaintenance.restart()
    print("SYSMAINT: Initiating restart")
    
    writefile(FILES.Restart, "1")
    
    local restarterScript = [[
-- RuptorX Temporary Restarter
task.wait(0.5)
loadfile("]] .. SCRIPT_PATH .. [[")()
]]
    
    writefile(FILES.TempRestarter, restarterScript)
    SystemMaintenance.stopIncrement()
    Interpreter.Active = false
    
    loadfile(FILES.TempRestarter)()
    
    task.wait(0.1)
    if isfile(FILES.TempRestarter) then delfile(FILES.TempRestarter) end
    if isfile(FILES.Restart) then delfile(FILES.Restart) end
end

-- Create failsafe script
function SystemMaintenance.createFailsafe()
    local failsafeScript = [[
-- RuptorX Failsafe Monitor
print("FAILSAFE: Started monitoring")

local FOLDER_PATH = "]] .. FOLDER_PATH .. [["
local INCREMENT_FILE = FOLDER_PATH .. "/increment.lock"
local SHUTDOWN_FILE = FOLDER_PATH .. "/shutdown.lock"
local RESTART_FILE = FOLDER_PATH .. "/restart.lock"
local SCRIPT_PATH = "]] .. SCRIPT_PATH .. [["

local lastIncrement = -1

while true do
    task.wait(1)
    
    if isfile(SHUTDOWN_FILE) or isfile(RESTART_FILE) then
        print("FAILSAFE: Shutdown/Restart detected, stopping monitor")
        break
    end
    
    if isfile(INCREMENT_FILE) then
        local currentIncrement = tonumber(readfile(INCREMENT_FILE)) or 0
        
        if lastIncrement == -1 then
            lastIncrement = currentIncrement
        elseif currentIncrement == lastIncrement then
            print("FAILSAFE: RuptorX hang/crash detected! Restarting...")
            task.wait(2)
            
            local recheckIncrement = tonumber(readfile(INCREMENT_FILE)) or 0
            if recheckIncrement == lastIncrement then
                loadfile(SCRIPT_PATH)()
                print("FAILSAFE: New RuptorX instance started")
                break
            end
        else
            lastIncrement = currentIncrement
...
print("FAILSAFE: Monitor stopped")
    ...
    writefile(FILES.Failsafe, failsafeScript)
    
    task.spawn(function()
        loadfile(FILES.Failsafe)()
    end)
    
    print("SYSMAINT: Failsafe monitor created and started")
end

-- Initialize system
function SystemMaintenance.init()
    print("SYSMAINT: Initializing System Maintenance...")
    SystemMaintenance.initFolders()
    SystemMaintenance.checkImproperShutdown()
    SystemMaintenance.initVersion()
    SystemMaintenance.startIncrement()
    SystemMaintenance.createFailsafe()
    SystemMaintenance.checkForUpdates()
    print("SYSMAINT: System Maintenance initialized")
end
...
Interpreter.Handlers.shutdown = function(flags)
    print("SYSMAINT: Preparing for shutdown")
    writefile(FILES.Shutdown, "1")
    SystemMaintenance.stopIncrement()
    originalShutdown(flags)
    task.wait(0.5)
    if isfile(FILES.Shutdown) then delfile(FILES.Shutdown) end
end
...
-- Refresh channels in dropdown
local function RefreshChatXChannels()
    if not ChatXGUI.ChannelList then return end
